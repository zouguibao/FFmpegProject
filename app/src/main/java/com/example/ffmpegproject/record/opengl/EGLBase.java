...
public class EGLBase {
    ...
    public static class EglSurface {
        ...
        public EglSurface(EGLBase mEgl, Object surface) {
            this.mEgl = mEgl;
            this.mEglSurface = mEgl.createWindowSurface(surface);
        }

        public EglSurface(EGLBase egl, int width, int height) {
            mEgl = egl;
    ...
    private void init(EGLContext shared_context, boolean with_depth_buffer, boolean isRecordable) {
        ...
        if (!EGL14.eglMakeCurrent(mEglDisplay, EGL14.EGL_NO_SURFACE, EGL14.EGL_NO_SURFACE, EGL14.EGL_NO_CONTEXT)) {
            throw new RuntimeException("mEglContext failed");
        }
        makeDefault();
    }
