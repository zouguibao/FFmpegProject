...
VAudio::VAudio(VideoStatus *playStatus, VideoJavaCall *javaCall1) {
    streamIndex = -1;
    out_buffer = (uint8_t *) malloc(sample_rate * 2 * 2 * 2 / 3);
    queue = new VideoFrameQueue(playStatus);
...
void pcmBufferCallBack(SLAndroidSimpleBufferQueueItf bf, void *context) {
    ...
    if (wlAudio != NULL) {
        ...
        if (wlAudio->buffer && wlAudio->pcmsize > 0) {
            ...
            wlAudio->javaCall->onVideoInfo(WL_THREAD_CHILD, wlAudio->clock, wlAudio->duration);
...
SLuint32 VAudio::getSLSampleRate() {
    switch (sample_rate) {
        case 8000:
...
void VAudio::setVideo(bool video) {
    isVideo = video;
}

void VAudio::setClock(int secds) {
    now_time = secds;
    clock = secds;
}