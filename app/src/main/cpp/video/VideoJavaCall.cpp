...
VideoJavaCall::VideoJavaCall(_JavaVM *vm, JNIEnv *env, jobject obj) {
    javaVM = vm;
    jniEnv = env;
    jobj = jniEnv->NewGlobalRef(obj);
...
VideoJavaCall::~VideoJavaCall() {

}

void VideoJavaCall::onError(int type, int code, const char *msg) {
...
void VideoJavaCall::onLoad(int type, bool load) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void VideoJavaCall::onParpared(int type) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void VideoJavaCall::onInitMediacodec(int type, int mimetype, int width, int height, int csd_0_size, int csd_1_size, uint8_t *csd_0, uint8_t *csd_1) {
...
void VideoJavaCall::onDecMediacodec(int type, int size, uint8_t *packet_data, int pts) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void VideoJavaCall::onGlRenderYuv(int type, int width, int height, uint8_t *fy, uint8_t *fu, uint8_t *fv) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void VideoJavaCall::onVideoInfo(int type, int currt_secd, int total_secd) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void VideoJavaCall::onComplete(int type) {
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
bool VideoJavaCall::isOnlySoft(int type) {
    bool soft = false;
    if(type == WL_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
