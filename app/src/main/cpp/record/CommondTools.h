...
#ifndef FFMPEGPROJECT_COMMONDTOOLS_H
#define FFMPEGPROJECT_COMMONDTOOLS_H
extern "C"{
#include <jni.h>
...
#define ACCOMPANY_PCM_OUTPUT_CHANNEL
...
static inline bool isAACSuffix(const char *accompanyPath) {
    bool ret = false;
    char *suffix = strrchr((char*)accompanyPath, '.');
...
static inline bool isPNGSuffix(const char *picFilePath) {
    bool ret = false;
    char *suffix = strrchr((char*)picFilePath, '.');
//	LOGI("suffix is %s", suffix);
...
static inline jint getCurrentTime() {
    struct timeval tv;
...
#endif //FFMPEGPROJECT_COMMONDTOOLS_H
